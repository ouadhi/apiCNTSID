...
public interface ContainersRepository extends JpaRepository<Container, Long>{
	...
	@Query("SELECT c FROM  Container  c  WHERE c.flag = false ")
	public List<Container>  getDataNotMarked () ;
	
	@Modifying
	@Transactional
	@Query(value = "update containers  set  flag='t' where id between :start and  :end  ", nativeQuery = true)
	public void setMareked(@Param("start") Long start ,@Param("end") Long end  ) ; 
	
	@Query("Select Count(*) from Container WHERE  flag  = false ")
	public int getCount () ; 
